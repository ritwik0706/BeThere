...
class CategorySelectionActivity : AppCompatActivity() {
    ...
    inner class SliderAdapter: PagerAdapter {
        ...
        var slideImages = intArrayOf(R.drawable.institute, R.drawable.prof, R.drawable.student)

        var categoryName = arrayOf("INSTITUTE", "PROFESSOR", "STUDENT")
        ...
        override fun instantiateItem(container: ViewGroup, position: Int): Any {
            ...
            if (categoryName[position]=="PROFESSOR"){
                ...
                myView.tvCategoryName.setOnClickListener {
                    var editor=prefs!!.edit()
                    editor.putString("Category","PROFESSOR")
                    editor.commit()
                    category=2
                    startActivityForResult(AuthUI.getInstance().createSignInIntentBuilder()
                            .setTosUrl(PATH_TOS)
    ...
    fun InstituteLogin(){
        var intent=Intent(this, InstitutePage::class.java)
        startActivity(intent)
        finish()
    }

    fun ProfLogin(){
        var intent=Intent(this,ProfessorActivity::class.java)
        startActivity(intent)
        finish()
    }
